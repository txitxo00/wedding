...
(function($) {
	...
	var targetDay = new Date ("10/16/2021");
	var today = new Date();
	var text=Math.ceil((targetDay.getTime()-today.getTime())/(1000*3600*24));
	if( text > 1 ){
		$("#sp").text(" y sólo quedan " + text + " días.");
	}else if(text == 1){
		$("#sp").text(" y sólo queda " + text + " día.");
	}else if(text == 0){
		$("#sp").text(" y es hoy!!");
	}else if(text<0){
		$("#sp").text(" y ya ha sido.");
	}
	//$("#sp").text(". Aunque hemos tenido que postponer de forma indefinida debido a la situación actual.");
